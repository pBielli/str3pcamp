...
export function updateSpotTypes(spotTypes) {
  SPOT_TYPES = spotTypes || {};
  buildLegend();
}
...
function buildLegend() {
  ...
  if (!legend) return;
  const rows = [`<div><span>👤</span> Sei qui</div>`];
  Object.values(SPOT_TYPES || {}).forEach(type => {
    const label = type.label || 'Tipo';
    const swatch = `<span style="display:inline-block;width:10px;height:10px;margin-right:8px;border-radius:50%;background:${type.color || '#4a7a4e'}"></span>`;
    rows.push(`<div>${swatch}${label}</div>`);
  });
