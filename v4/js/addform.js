...
export function renderAddStep() {
  ...
  const locale       = cfg().app?.locale || 'it';
  ...
  const availableVehicles = Object.entries(VEHICLES);
  const showVehicleSection = addForm.spotType !== 'accoglienza' && availableVehicles.length;

  if (addStep === 1) {
    const typeItems = Object.entries(SPOT_TYPES).map(([key, item]) => {
      const icon = getTypeIcon(key);
      return `<div class="type-card${addForm.spotType === key ? ' on' : ''}" data-type="${key}">
        <div class="tc-icon">${icon}</div>
        <div class="tc-label">${esc(item.label || key)}</div>
      </div>`;
    }).join('');

    const vehicleItems = availableVehicles.map(([key, item]) => {
      return `<div class="veh-btn${addForm.vehicles.includes(key) ? ' on' : ''}" data-veh="${key}">${item.emoji || '•'}<div class="vl">${esc(item.label)}</div></div>`;
    }).join('');
    ...
    c.innerHTML = `
      ...
      <div class="type-grid">${typeItems}</div>

      <div id="veh-section" style="${showVehicleSection ? '' : 'display:none'}">
        <label class="flabel">TIPO DI VEICOLO ACCETTATO</label>
        <div class="veh-row">${vehicleItems}</div>
      </div>
    ...
    document.querySelectorAll('.type-card').forEach(card => {
      card.onclick = () => setSpotTypeLocal(card.dataset.type);
    });
    document.querySelectorAll('.veh-btn').forEach(btn => {
      btn.onclick = () => togVehLocal(btn.dataset.veh);
    });
    ...
    const { element } = createPlacesSearch({
      placeholder: locale === 'en' ? '🔍 Search on map...' : '🔍 Cerca luogo su mappa...',
      locale,
      onSelect: (place) => {
        ...
        window.toast && window.toast(locale === 'en' ? 'Place imported! Check details.' : 'Luogo importato! Controlla e modifica se necessario 📍', 'info');
      }
    });
    ...
    const SPOT_TYPES_LABELS = SPOT_TYPES;
    c.innerHTML = `
...
function getTypeIcon(type) {
  if (type === 'accoglienza') return '🏠';
  if (type === 'camping') return '⛺💰';
  if (type === 'libero') return '⛺🌲';
  return '🏕️';
}
...
function setSpotTypeLocal(t) {
  addForm.spotType = t;
  document.querySelectorAll('.type-card').forEach(card => {
    card.classList.toggle('on', card.dataset.type === t);
  });
...
function togVehLocal(v) {
  addForm.vehicles = addForm.vehicles.includes(v)
    ...
    : [...addForm.vehicles, v];
  const btn = document.querySelector(`[data-veh="${v}"]`);
